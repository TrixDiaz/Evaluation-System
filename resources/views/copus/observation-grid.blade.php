    ...
    <div class="overflow-x-auto rounded-lg border border-gray-200 dark:border-gray-700 shadow">
        <table class="w-full border-collapse">
            ...
            <tr class="bg-gray-100 dark:bg-gray-800 text-sm">
                <th class="border-b border-r border-gray-200 dark:border-gray-700 p-2 text-left w-20 font-medium">min</th>
                <th colspan="{{ count($studentActivities) }}" class="border-b border-gray-200 dark:border-gray-700 p-2 text-center bg-blue-50 dark:bg-blue-900 font-medium">
                    1. Students doing
                </th>
                <th colspan="{{ count($instructorActivities) }}" class="border-b border-gray-200 dark:border-gray-700 p-2 text-center bg-green-50 dark:bg-green-900 font-medium">
                    2. Instructor doing
                </th>
                <th rowspan="2" class="border-b border-l border-gray-200 dark:border-gray-700 p-2 text-left font-medium w-72">
                    Comments: <span class="font-normal text-gray-500 dark:text-gray-400">EG: explain difficult concepts, analogies, etc.</span>
                </th>
            </tr>
            <tr class="bg-gray-50 dark:bg-gray-700 text-xs">
                <th class="border-r border-b border-gray-200 dark:border-gray-700 p-2 text-center font-medium">Time</th>
                @foreach(array_keys($studentActivities) as $code)
                    <th class="border-r border-b border-gray-200 dark:border-gray-700 p-2 text-center" title="{{ $studentActivities[$code] }}">
                        {{ $code }}
                    </th>
                @endforeach
                @foreach(array_keys($instructorActivities) as $code)
                    <th class="border-r border-b border-gray-200 dark:border-gray-700 p-2 text-center" title="{{ $instructorActivities[$code] }}">
                        {{ $code }}
                    </th>
                @endforeach
            </tr>
            ...
            @foreach($timeSegments as $time)
                <tr class="hover:bg-gray-400/10 dark:hover:bg-gray-900 hover:dark:text-black transition-colors">
                    <td class="border-r border-b border-gray-200 dark:border-gray-700 p-2 text-center font-medium">{{ $time }}</td>
                    ...
                    <td class="border-b border-gray-200 dark:border-gray-700 p-2">
                        <button type="button"
                                ...
                                <span x-data="{ hasComment: false }" x-init="hasComment = $refs.commentInput.value !== ''" x-effect="hasComment = $refs.commentInput.value !== ''">
                                    <span x-show="!hasComment">Add comment...</span>
                                    <span x-show="hasComment" x-text="$refs.commentInput.value.length > 40 ? $refs.commentInput.value.substring(0, 37) + '...' : $refs.commentInput.value"></span>
                                </span>
                            <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4 ml-2" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z" />
                            </svg>
                        </button>
                        <textarea x-ref="commentInput" name="comments[{{ $time }}]" class="hidden"></textarea>
                        ...
                        >
                            <div class="flex items-center justify-center min-h-screen p-4 text-center sm:p-0">
                                <div x-on:click="isOpen = false" class="fixed inset-0 transition-opacity bg-gray-500 dark:bg-gray-900 bg-opacity-75 dark:bg-opacity-75"></div>

                                <div class="relative bg-white dark:bg-gray-800 rounded-lg text-left overflow-hidden shadow-xl transform transition-all sm:my-8 sm:max-w-lg sm:w-full p-6">
                                    <div class="px-4 pt-5 pb-4 sm:p-6 sm:pb-4">
                                        ...
                                        <div class="mt-4">
                                                <textarea
                                                    x-on:input="$refs.commentInput.value = $event.target.value"
                                                    ...
                                                    rows="6"
                                                    placeholder="Enter detailed observations, explanations, difficulties, analogies, etc."
                                                    x-bind:value="$refs.commentInput.value"
                                                ></textarea>
                                        </div>
                                    </div>
